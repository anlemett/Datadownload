...
def get_states_inside_TMA(states_df, month, week):
    ...
    filename = 'osn_' + airport_icao + '_states_TMA_raw_all_' + year + '_' + month + '_week' + str(week) + '.csv'
    if not arrival:
    ...
    for flight_id, flight_df in states_df.groupby(level='flightId'):
        ...
        new_df_inside_TMA = new_df_inside_TMA[['sequence', 'timestamp', 'lat', 'lon', 'altitude', 'velocity', 'endDate']]
